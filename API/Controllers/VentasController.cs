...
{
    [Authorize]
    public class VentasController(IVentaService ventaService) : BaseApiController
    {
        ...
        {
            var ventas = await ventaService.GetVentasDelDiaAsync(KioscoId, cancellationToken, fecha);
        ...
        /// </summary>
        /// <param name="cancellationToken">Token para cancelar la operación.</param>
        /// <param name="cantidad">Cantidad de ventas a obtener (por defecto 4, debe ser mayor que 0).</param>
        /// <returns>Lista de las ventas más recientes.</returns>
        /// <response code="200">Retorna la lista de ventas más recientes.</response>
        /// <response code="400">Cantidad inválida (debe ser mayor que 0).</response>
        /// <response code="401">No autorizado. Se requiere un JWT válido.</response>
        [ProducesResponseType(typeof(IReadOnlyList<VentaDto>), StatusCodes.Status200OK)]
        ...
        {
            var ventas = await ventaService.GetVentasRecientesAsync(KioscoId, cantidad, cancellationToken);
            return ventas.ToActionResult();
        ...
        {
            var total = await ventaService.GetMontoTotalVentasDelDiaAsync(KioscoId, cancellationToken, fecha);
        ...
        /// <param name="cancellationToken">Token para cancelar la operación.</param>
        /// <param name="cantidad">Cantidad de productos a obtener (por defecto 4, debe ser mayor que 0).</param>
        ...
        public async Task<ActionResult<IReadOnlyList<ProductoMasVendidoDto>>> GetProductosMasVendidosDelDia(
            ...
            [FromQuery] int cantidad = 4)
        {
            var productos = await ventaService.GetProductosMasVendidosDelDiaAsync(KioscoId, cantidad, cancellationToken, fecha);
        ...
        {
            var result = await ventaService.CreateVentaAsync(ventaDto, KioscoId, UserId, cancellationToken);
        ...
        {
            var result = ventaService.GetVentasForExportAsync(KioscoId, cancellationToken, fechaInicio, fechaFin, limite);
